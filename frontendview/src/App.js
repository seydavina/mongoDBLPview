    ...
    <div className="container mt-4">
      ...
      <div className="mb-3 row">
        ...
        <div className="col-md-6 d-flex justify-content-end">
          <select className="form-select me-2" value={sort} onChange={handleSortChange}>
          ...
          <ul className="list-group">
            {authors.map(publi => (
              <li key={publi._id} className="list-group-item">
                <h5>{publi.title}</h5>
                <p><strong>Auteurs:</strong> {publi.authors.join(', ')}</p>
                <p><strong>Année:</strong> {publi.year}</p>
                <p><strong>Livre:</strong> {publi.booktitle}</p>
                <a href={`http://dblp.uni-trier.de/${publi.url}`} target="_blank" rel="noopener noreferrer" className="btn btn-primary">Voir Plus</a>
              </li>
            ))}
          </ul>
        </div>
